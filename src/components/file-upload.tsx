...
export function FileUpload({ onFileLoaded }: FileUploadProps) {
  ...
  return (
    ...
    >
      <label className="cursor-pointer block">
        <div className="flex flex-col items-center">
          ...
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-all ${
            ...
            {isLoading ? (
              <svg className="w-7 h-7 text-stone-400 animate-spin" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
            ) : (
              <svg className={`w-7 h-7 transition-colors ${isDragOver ? 'text-indigo-500' : 'text-stone-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <p className="text-[14px] text-stone-400">
            or <span className="text-indigo-500 hover:text-indigo-600 font-medium">browse</span> to select a JSON file
          </p>
        </div>
        <input
          type="file"
      ...
      {error && (
        <p className="mt-4 text-[13px] text-red-600 bg-red-50 px-4 py-2 rounded-lg font-medium">{error}</p>
      )}
    </div>
  );
}
